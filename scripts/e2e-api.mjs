...
const redactImageForm = new FormData()
redactImageForm.append('file', new Blob([imageInput], { type: 'image/png' }), 'sensitive.png')
redactImageForm.append('regions', JSON.stringify([{ x: 25, y: 25, w: 50, h: 50 }]))
redactImageForm.append('redactionColor', '#111827')
redactImageForm.append('format', 'png')
const redactedImage = await request('/api/tools/image/redact', redactImageForm)
assert.equal(redactedImage.response.headers.get('x-redaction-regions'), '1')
assert.equal(redactedImage.response.headers.get('x-metadata-stripped'), 'yes')
const { data: redactedPixels, info: redactedInfo } = await sharp(redactedImage.body).ensureAlpha().raw().toBuffer({ resolveWithObject: true })
const redactedPixel = (x, y) => [...redactedPixels.subarray((y * redactedInfo.width + x) * 4, (y * redactedInfo.width + x) * 4 + 4)]
assert.deepEqual(redactedPixel(40, 30), [17, 24, 39, 255], 'Pixel trong vùng nhạy cảm phải bị ghi đè bằng màu đặc.')
assert.deepEqual(redactedPixel(5, 5), [35, 120, 210, 255], 'Pixel ngoài vùng che phải giữ nguyên màu nguồn.')

const invalidRedactionForm = new FormData()
invalidRedactionForm.append('file', new Blob([imageInput], { type: 'image/png' }), 'sensitive.png')
invalidRedactionForm.append('regions', JSON.stringify(Array.from({ length: 21 }, () => ({ x: 1, y: 1, w: 5, h: 5 }))))
const invalidRedaction = await requestError('/api/tools/image/redact', invalidRedactionForm, 400)
assert.match(invalidRedaction.message, /20 vùng che/)
