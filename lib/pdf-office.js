...
const createHeaderTable = (lines, page, layout) => {
  ...
  const rows = grouped.map((groups, index) => new TableRow({
    height: { value: pointToTwip(index === grouped.length - 1 ? 18 : 16), rule: HeightRule.ATLEAST },
...
const createFlowParagraph = (block, page, layout, blockIndex = 0) => {
  ...
  return new Paragraph({
    ...
    spacing: {
      before: pointToTwip(blockIndex ? 7 : 5),
      after: 0,
      line: 276,
...
const createFooterTable = (lines, page, layout) => {
  ...
  const paragraphForFooterLine = (line, index, side) => createCellParagraph(line.runs, {
    alignment: side === 'right' ? AlignmentType.CENTER : AlignmentType.LEFT,
    before: side === 'right' && index === rightLines.length - 1 && rightLines.length > 1 ? 42 : 0,
    line: 240,
  })
  return new Table({
    width: { size: pointToTwip(contentWidth), type: WidthType.DXA },
...
const createStructuredWordContent = (page, layout) => {
  ...
  const addFlow = lines => groupFlowLines(lines, page, layout).forEach((block, index) => children.push(createFlowParagraph(block, page, layout, index)))
  if (table) {
    addFlow(page.lines.slice(contentStart, table.startIndex))
    children.push(new Paragraph({ spacing: { before: pointToTwip(7), after: pointToTwip(3) }, children: [new TextRun('')] }))
    children.push(table.element)
    addFlow(page.lines.slice(table.endIndex + 1, contentEnd))
...
const createWord = async (pages, source) => {
  ...
  return { buffer: await Packer.toBuffer(document), detectedTables }
}
...
export const convertPdfText = async (buffer, format) => {
  ...
  if (format === 'word') {
    ...
    return { buffer: word.buffer, detectedTables: word.detectedTables, extension: 'docx', type: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', layoutMode: 'flowing-reconstruction', ...common }
  }
