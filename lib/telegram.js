...
export class TelegramBotManager {
      ...
      await fetch(`${this.baseUrl}/setMyCommands`, {
        ...
        body: JSON.stringify({
          commands: [
            ...
            { command: 'week', description: '🗓️ Báo cáo & biểu đồ 7 ngày qua' },
            { command: 'month', description: '📆 Báo cáo & biểu đồ 30 ngày qua' },
            ...
            { command: 'blacklist', description: '🚫 Xem danh sách các IP đang bị chặn' },
  ...
  getReplyKeyboard() {
    return {
      keyboard: [
        ...
        [{ text: '🗓️ 7 ngày qua' }, { text: '📆 30 ngày qua' }],
        [{ text: '🏆 Xếp hạng' }, { text: '⏱️ Gần đây' }],
        [{ text: '🖥️ VPS' }, { text: '🚫 Danh sách chặn' }],
  ...
  getInlineKeyboard(currentCmd = 'stats') {
    return {
      inline_keyboard: [
        ...
        [
          { text: '🗓️ 7 ngày', callback_data: 'week' },
          { text: '📆 30 ngày', callback_data: 'month' },
        ],
        [
          { text: '📊 Tổng quan', callback_data: 'stats' },
  ...
  notifyToolUsage(event) {
    ...
    const geoText = event.geoLabel ? ` (${event.geoLabel})` : ''

    const message = [
      `🔔 <b>[PDFTools] LƯỢT DÙNG CÔNG CỤ</b>`,
      `──────────────────────────`,
      `⏱️ <b>Thời gian:</b> ${dateStr}`,
      `🌐 <b>Địa chỉ IP:</b> <code>${event.ip || '127.0.0.1'}</code>${geoText}`,
  ...
  notifySpamAlert({ ip, count, geoLabel, tool }) {
    if (!this.isConfigured()) return
    const nowStr = new Date().toLocaleTimeString('vi-VN', { hour: '2-digit', minute: '2-digit', second: '2-digit' })
    const geoText = geoLabel ? ` (${geoLabel})` : ''
    const message = [
      `🚨 <b>[CẢNH BÁO SPAM / TẢI NẶNG]</b>`,
      `──────────────────────────`,
      `⏱️ <b>Thời điểm:</b> ${nowStr}`,
      `🌐 <b>Địa chỉ IP:</b> <code>${ip}</code>${geoText}`,
      `⚡ <b>Tần suất:</b> <b>${count} lượt gọi</b> trong vòng 60 giây!`,
      `🔧 <b>Công cụ gần nhất:</b> <b>${tool}</b>`,
      ``,
      `👉 <i>Để chặn IP này ngay lập tức, hãy bấm hoặc gửi:</i>`,
      `<code>/block ${ip}</code>`,
    ].join('\n')
  ...
  _resolveCommand(text) {
    ...
    if (t.startsWith('/week') || t.startsWith('/7days') || t.includes('7 ngày') || t.includes('1 tuần') || t === 'week') return 'week'
    if (t.startsWith('/month') || t.startsWith('/30days') || t.includes('30 ngày') || t.includes('1 tháng') || t === 'month') return 'month'
    ...
    if (t.startsWith('/blacklist') || t.includes('danh sách đen') || t.includes('bị chặn') || t === 'blacklist') return 'blacklist'
  ...
  async _handleIncomingMessage(msg) {
    ...
    const cmdFirst = parts[0].toLowerCase()
    ...
    if (cmdFirst === '/block') {
      ...
      if (!targetIp) {
        await this.sendMessage('⚠️ <b>Thiếu địa chỉ IP</b>\nCú pháp: <code>/block &lt;địa_chỉ_ip&gt;</code>\nVí dụ: <code>/block 113.161.42.18</code>', { chatId: fromChatId })
        return
      }
      const result = await this.analyticsTracker.blockIp(targetIp, 'Lệnh Telegram Bot')
      if (!result) {
        await this.sendMessage(`⚠️ Không thể chặn IP <code>${targetIp}</code> (đây là IP nội bộ/localhost hoặc không hợp lệ).`, { chatId: fromChatId })
      } else {
        await this.sendMessage(
          `🚫 <b>ĐÃ CHẶN ĐỊA CHỈ IP</b>\n──────────────────────────\n🌐 <b>IP:</b> <code>${targetIp}</code>\n${result.geoLabel ? `📍 <b>Vị trí:</b> ${result.geoLabel}\n` : ''}⏱️ <b>Thời gian:</b> ${new Date().toLocaleTimeString('vi-VN')}\n\n<i>Mọi yêu cầu API từ IP này sẽ bị từ chối truy cập (403 Forbidden).</i>\n👉 <i>Để gỡ chặn, hãy gửi:</i> <code>/unblock ${targetIp}</code>`,
          { chatId: fromChatId }
        )
      }
      return
    ...
    // Xử lý lệnh /unblock <IP>
    if (cmdFirst === '/unblock') {
      if (!this.analyticsTracker) return
      if (!targetIp) {
        await this.sendMessage('⚠️ <b>Thiếu địa chỉ IP</b>\nCú pháp: <code>/unblock &lt;địa_chỉ_ip&gt;</code>\nVí dụ: <code>/unblock 113.161.42.18</code>', { chatId: fromChatId })
        return
      }
      const success = await this.analyticsTracker.unblockIp(targetIp)
      if (success) {
        await this.sendMessage(
          `✅ <b>ĐÃ GỠ CHẶN ĐỊA CHỈ IP</b>\n──────────────────────────\n🌐 <b>IP:</b> <code>${targetIp}</code>\n\n<i>IP này hiện đã có thể truy cập và sử dụng lại các công cụ bình thường.</i>`,
          { chatId: fromChatId }
        )
      } else {
        await this.sendMessage(`ℹ️ IP <code>${targetIp}</code> hiện không nằm trong danh sách chặn.`, { chatId: fromChatId })
      }
      return
    }
  ...
  async _dispatchCommand(command, fromChatId, isCallback = false) {
    ...
    if (command === 'help') {
      const helpMsg = [
        ...
        `🗓️ <b>7 ngày:</b> Biểu đồ & thống kê 1 tuần qua`,
        `📆 <b>30 ngày:</b> Biểu đồ & thống kê 1 tháng qua`,
        `🖥️ <b>VPS:</b> Cấu hình CPU, RAM, Ổ cứng Disk dạng biểu đồ`,
        `🚫 <b>Danh sách chặn:</b> Các IP đang bị chặn truy cập`,
        `🏆 <b>Xếp hạng:</b> Top công cụ & Top IP`,
        `⏱️ <b>Gần đây:</b> 8 hoạt động mới nhất`,
        `🏓 <b>Máy chủ:</b> Uptime & tình trạng Node.js`,
        ``,
        `👉 <i>Lệnh trực tiếp:</i>`,
        `• <code>/block &lt;ip&gt;</code>: Chặn ngay 1 địa chỉ IP spam`,
        `• <code>/unblock &lt;ip&gt;</code>: Mở chặn cho địa chỉ IP`,
      ].join('\n')
      ...
      const statsMsg = [
        ...
        `🚫 <b>IP đang bị chặn:</b> <b>${s.totalBlockedIps || 0}</b> IP`,
      ...
      const wStats = this.analyticsTracker.getStats({ range: '7days', limit: 8 })
      const ws = wStats.summary || {}
      const maxDayTotal = Math.max(...(wStats.dailyTrend || []).map(d => d.total), 1)
      const trendLines = (wStats.dailyTrend || []).map(d => {
        const pct = Math.round((d.total / maxDayTotal) * 100)
        return `• <b>${d.label}</b> (<code>${d.dateKey.slice(5)}</code>): <b>${d.total} lượt</b> (👁️ ${d.visits} · 🔧 ${d.toolUses})\n  └─ [<code>${makeProgressBar(pct, 8)}</code>]`
      }).join('\n')

      const topToolsList = (wStats.topTools || []).slice(0, 5)
        .map(t => `• <b>${t.tool}</b>: <b>${t.totalUses} lượt</b>`)
        .join('\n') || 'Chưa có công cụ'

      const topIpsList = (wStats.topIps || []).slice(0, 5)
        .map((ip, idx) => `${idx + 1}. <code>${ip.ip}</code>: ${ip.visits + ip.toolUses} lượt (🔧 ${ip.toolUses})`)
        .join('\n') || 'Chưa có IP'

      const weekMsg = [
        `🗓️ <b>BÁO CÁO HOẠT ĐỘNG 7 NGÀY QUA</b>`,
        `──────────────────────────`,
        `👁️ <b>Lượt truy cập tuần:</b> <b>${ws.totalVisits || 0}</b>`,
        `⚡ <b>Lượt dùng công cụ:</b> <b>${ws.totalToolUses || 0}</b>`,
        `🌐 <b>Địa chỉ IP duy nhất:</b> <b>${ws.uniqueIps || 0} IP</b>`,
        `\n📊 <b>BIỂU ĐỒ XU HƯỚNG TỪNG NGÀY:</b>`,
        trendLines,
        `\n🏆 <b>TOP CÔNG CỤ TRONG TUẦN:</b>`,
        topToolsList,
        `\n🌐 <b>TOP IP HOẠT ĐỘNG TRONG TUẦN:</b>\n${topIpsList}`,
        `\n⏱️ <i>Cập nhật lúc: ${nowStr}</i>`,
      ].join('\n')

      await this.sendMessage(weekMsg, {
        chatId: fromChatId,
        reply_markup: isCallback ? this.getInlineKeyboard('week') : this.getReplyKeyboard(),
      })
    } else if (command === 'month') {
      const mStats = this.analyticsTracker.getStats({ range: '30days', limit: 8 })
      const ms = mStats.summary || {}
      const maxDayTotal = Math.max(...(mStats.dailyTrend || []).map(d => d.total), 1)
      const recent7Days = (mStats.dailyTrend || []).slice(-7)
      const trendLines = recent7Days.map(d => {
        const pct = Math.round((d.total / maxDayTotal) * 100)
        return `• <b>${d.label}</b> (<code>${d.dateKey.slice(5)}</code>): <b>${d.total} lượt</b> (👁️ ${d.visits} · 🔧 ${d.toolUses})\n  └─ [<code>${makeProgressBar(pct, 8)}</code>]`
      }).join('\n')

      const topToolsList = (mStats.topTools || []).slice(0, 6)
        .map(t => `• <b>${t.tool}</b>: <b>${t.totalUses} lượt</b>`)
        .join('\n') || 'Chưa có công cụ'

      const topIpsList = (mStats.topIps || []).slice(0, 6)
        .map((ip, idx) => `${idx + 1}. <code>${ip.ip}</code>: ${ip.visits + ip.toolUses} lượt (🔧 ${ip.toolUses})`)
        .join('\n') || 'Chưa có IP'

      const monthMsg = [
        `📆 <b>BÁO CÁO HOẠT ĐỘNG 30 NGÀY QUA</b>`,
        `──────────────────────────`,
        `👁️ <b>Lượt truy cập tháng:</b> <b>${ms.totalVisits || 0}</b>`,
        `⚡ <b>Lượt dùng công cụ:</b> <b>${ms.totalToolUses || 0}</b>`,
        `🌐 <b>Địa chỉ IP duy nhất:</b> <b>${ms.uniqueIps || 0} IP</b>`,
        `\n📊 <b>BIỂU ĐỒ 7 NGÀY GẦN NHẤT CỦA THÁNG:</b>`,
        trendLines,
        `\n🏆 <b>TOP CÔNG CỤ TRONG THÁNG:</b>`,
        topToolsList,
        `\n🌐 <b>TOP IP HOẠT ĐỘNG TRONG THÁNG:</b>\n${topIpsList}`,
        `\n⏱️ <i>Cập nhật lúc: ${nowStr}</i>`,
      ].join('\n')

      await this.sendMessage(monthMsg, {
        chatId: fromChatId,
        reply_markup: isCallback ? this.getInlineKeyboard('month') : this.getReplyKeyboard(),
      })
    } else if (command === 'vps') {
      ...
      const ipsList = (stats.topIps || []).slice(0, 6)
        .map((ip, i) => {
          const flag = ip.geo?.flag || '🌐'
          const blockBadge = ip.isBlocked ? ' 🚫' : ''
          return `${i + 1}. <code>${ip.ip}</code> ${flag}${blockBadge}: ${ip.toolUses} dùng, ${ip.visits} truy cập`
        })
        .join('\n') || 'Chưa có dữ liệu'
      ...
      const blockedList = this.analyticsTracker.getBlockedIps()
      if (!blockedList.length) {
        await this.sendMessage(
          '🛡️ <b>DANH SÁCH ĐEN IP (BLACKLIST)</b>\n──────────────────────────\nHiện tại <b>không có</b> địa chỉ IP nào bị chặn.',
          {
            chatId: fromChatId,
            reply_markup: this.getReplyKeyboard(),
          }
        ...
        const lines = blockedList.map((item, idx) => {
          const time = new Date(item.blockedAt).toLocaleString('vi-VN', {
            hour: '2-digit', minute: '2-digit', day: '2-digit', month: '2-digit',
          })
          return `${idx + 1}. <code>${item.ip}</code> ${item.geoLabel ? `(${item.geoLabel})` : ''}\n   └ <i>Lý do:</i> ${item.reason}\n   └ <i>Thời gian:</i> ${time}\n   └ <i>Gỡ chặn:</i> <code>/unblock ${item.ip}</code>`
        })
        await this.sendMessage(
          `🚫 <b>DANH SÁCH IP ĐANG BỊ CHẶN (${blockedList.length})</b>\n──────────────────────────\n${lines.join('\n\n')}\n\n👉 <i>Để gỡ chặn, hãy gửi lệnh:</i> <code>/unblock &lt;ip&gt;</code>`,
          {
            chatId: fromChatId,
            reply_markup: this.getReplyKeyboard(),
          }
      ...
      const eventsList = (stats.recentEvents || []).slice(0, 8).map(e => {
        ...
        const flag = e.geo?.flag || ''
        return `• [${time}] <code>${e.ip}</code> ${flag} ${icon} <b>${label}</b> ${status}`
