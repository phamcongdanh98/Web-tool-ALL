...
export const getAdminMetadata = () => {
  ...
  const newProvincesRaw = db.prepare(`
  ...
  // Khử trùng lặp theo key chuẩn hóa để luôn hiển thị đúng 34 tỉnh/thành mới
  const newProvMap = new Map()
  for (const p of newProvincesRaw) {
    if (!newProvMap.has(p.key)) {
      newProvMap.set(p.key, p)
    }
  }
  const newProvinces = Array.from(newProvMap.values())
