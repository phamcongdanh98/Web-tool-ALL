...
export const initDatabase = (db) => {
  ...
  // 7. ƯU TIÊN SỐ 1: Nạp mappings cấp xã từ Thư Viện Pháp Luật (data/tvpl-extracted-wards.json)
  const tvplPath = path.join(rootDir, 'data', 'tvpl-extracted-wards.json')
  const processedOldKeys = new Set()

  if (fs.existsSync(tvplPath)) {
    try {
      const tvplData = JSON.parse(fs.readFileSync(tvplPath, 'utf8'))
      db.exec('BEGIN TRANSACTION;')

      // Đăng ký các legalDocs nếu chưa có
