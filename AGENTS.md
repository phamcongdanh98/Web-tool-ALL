...
- PDF → Word có hai hợp đồng không được nhập nhằng. **Bố cục chính xác** là mặc định: chạy trong browser, PDF.js bỏ các toán tử vẽ chữ khỏi nền 200 DPI rồi `lib/exact-word.js` dựng mỗi text item thành `WpsShapeRun`/text box Word theo tọa độ trang, font, cỡ, đậm/nghiêng, màu và độ co ngang; đường kẻ, ảnh, dấu và chữ ký còn lại ở nền. Đúng khổ/lề 0, tối đa 40 trang và chỉ nhận PDF có lớp chữ. PDF scan hoặc trang ảnh phải hướng dẫn OCR, không fallback sang ảnh toàn trang rồi gọi là “chỉnh sửa được”. Giữ lazy-load `docx`, giới hạn pixel/trang, chọn PNG/JPEG theo dung lượng và cleanup canvas/page/loading task.
- **Dòng chảy văn bản** cùng Excel/PowerPoint/TXT chỉ xử lý văn bản có thể chọn, tối đa 100 trang. PDF scan phải trả 422 và hướng dẫn OCR; PDF hỗn hợp phải báo số trang ảnh chưa OCR, không fallback sang tệp Office rỗng. Có thể dùng Creator/Producer để ghi “có dấu hiệu xuất từ Word”; PDF có `/ByteRange` hoặc trường `/Sig` phải được báo là tài liệu đã ký, nhưng không được khẳng định khôi phục file Word gốc hay giữ hiệu lực chữ ký trong DOCX. Word dòng chảy giữ khổ/ngắt trang, gom dòng thành đoạn, phục hồi bố cục hai cột và nhận diện bảng `STT` thành bảng Word thật. Bố cục vẫn là gần đúng và phải được mô tả trung thực trên UI.
- Khi sửa PDF → Word, E2E exact-mode phải kiểm tra `wps:wsp`, `w:txbxContent`, chữ semantic trong `document.xml`, neo theo trang, `pgSz`, lề 0 và chỉ media nền cho mỗi trang; render LibreOffice phải xác nhận không có chữ trùng, mất cuối dòng, trang trắng hoặc lệch cạnh. Fixture thật phải có chữ tiếng Việt, đậm/nghiêng, màu, đường kẻ, dấu/chữ ký. Không tuyên bố ngang bằng Smallpdf cho mọi PDF: font nhúng độc quyền có thể bị thay thế và Exact-mode ưu tiên vị trí hơn khả năng reflow. Với Word dòng chảy dạng công văn, ngoài semantic OOXML phải kiểm tra số trang, hai cột tiêu đề, bảng, nơi nhận và khối ký tên. Không dùng OCR cho PDF đã có lớp chữ chỉ vì metadata Word bị phần mềm ký số ghi đè.
