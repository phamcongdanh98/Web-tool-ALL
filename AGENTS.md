...
- Mỗi lần tải hoặc reload ứng dụng, màn hình chào thương hiệu hiển thị khoảng 4,5 giây, bắt đầu mờ dần ở 4,1 giây. Splash dùng chính `public/favicon.svg` làm lõi logo nhiều lớp với quỹ đạo/thẻ công cụ 3D và tên `Danh Phạm` chạy từng ký tự; không thêm thư viện hoặc asset ngoài. Nội dung liên hệ phải dùng chung `footerContacts`, timer và khóa cuộn phải được cleanup khi unmount, giao diện phải responsive, tôn trọng `prefers-reduced-motion` và không tải thêm asset/CDN ngoài logo có sẵn.
