...
- Mỗi lần tải hoặc reload ứng dụng, màn hình chào thương hiệu chạy chuỗi cảnh khoảng 6,7 giây: file PDF → particle/4 engine → `Công Cụ Web` → signature `Danh Phạm` → logo bay về header; bắt đầu mờ ở 6,1 giây và có nút `Bỏ qua`. Splash chỉ dùng CSS cùng `public/favicon.svg`, không thêm thư viện/asset ngoài; timer, khóa cuộn và skip phải cleanup đúng, responsive, còn `prefers-reduced-motion` rút xuống khoảng 1,3 giây và bỏ particle/quỹ đạo.
- Nhận diện cá nhân dùng hệ ba tầng: `by Danh Phạm` nhỏ và bền vững ở header/modal, monogram `DP` làm họa tiết phụ trên card, còn signature lớn chỉ dành cho splash và khối `#creator`. Giữ cùng gradient cyan–indigo–tím, không lặp tên thành watermark đậm che nội dung; metadata, manifest, hero, creator showcase và footer phải dùng thống nhất tên `Danh Phạm`.
