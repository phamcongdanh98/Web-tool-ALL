...
- Typography desktop dùng font hệ thống trước để hiển thị sắc nét trên macOS/Windows. Không đưa lại cỡ chữ nội dung/chú thích quan trọng xuống 7–10 px. Hệ scale giữ layout gọn dưới 1680px, tăng typography/control/icon từ 1680×850 và tăng thêm ở 2300×1200; không dùng `zoom` hoặc `transform` cho toàn trang. Sau thay đổi giao diện diện rộng phải kiểm tra 1366×768, 1920×1080, 2560×1440 và mobile 390×844 để tránh chữ nhỏ, modal hẹp hoặc tràn ngang.
...
- Sau mỗi công việc làm thay đổi code, cấu hình hoặc dependency, cập nhật mục **Nhật ký phiên bản** trong `README.md`. Mốc hiện tại dùng `<details open>`, mốc cũ thu gọn; ghi ngắn gọn phiên bản/ngày, nội dung đã làm, kiểm thử đã chạy và lưu ý cần thiết khi mở dự án trên máy khác.
