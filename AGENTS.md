...
- PDF sang Word/Excel/PowerPoint/TXT chỉ xử lý văn bản có thể chọn, tối đa 100 trang. PDF scan phải trả 422 và hướng dẫn OCR; PDF hỗn hợp phải báo số trang ảnh chưa OCR, không fallback sang tệp Office rỗng. Có thể dùng Creator/Producer để ghi “có dấu hiệu xuất từ Word”; PDF có `/ByteRange` hoặc trường `/Sig` phải được báo là tài liệu đã ký, nhưng không được khẳng định khôi phục file Word gốc hay giữ hiệu lực chữ ký trong DOCX. Word dùng tái dựng dòng chảy: giữ khổ/ngắt trang, gom dòng thành đoạn, phục hồi bố cục hai cột và nhận diện bảng `STT` thành bảng Word thật có độ rộng, border, ô gộp và căn lề rõ ràng. Bố cục vẫn là gần đúng và phải được mô tả trung thực trên UI.
- Khi sửa PDF → Word dạng công văn, ngoài semantic OOXML phải render DOCX bằng LibreOffice và kiểm tra số trang, hai cột tiêu đề, bảng, nơi nhận và khối ký tên. Không dùng OCR cho PDF đã có lớp chữ chỉ vì metadata Word bị phần mềm ký số ghi đè.
