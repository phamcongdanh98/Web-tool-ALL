...
- PDF → Word có hai hợp đồng không được nhập nhằng. **Giữ nguyên hình thức** là mặc định: chạy trong browser, PDF.js render nội dung cùng annotation ở 200 DPI rồi `lib/exact-word.js` đặt mỗi ảnh vào một trang Word đúng khổ/lề 0; tối đa 40 trang. Chế độ này nhận cả PDF scan, giữ phần nhìn thấy của dấu/chữ ký nhưng chữ không sửa riêng được và chữ ký số không còn hiệu lực xác thực. Giữ lazy-load `docx`, giới hạn pixel/trang, chọn PNG/JPEG theo dung lượng và cleanup canvas/page/loading task.
- **Word có thể chỉnh sửa** cùng Excel/PowerPoint/TXT chỉ xử lý văn bản có thể chọn, tối đa 100 trang. PDF scan phải trả 422 và hướng dẫn OCR; PDF hỗn hợp phải báo số trang ảnh chưa OCR, không fallback sang tệp Office rỗng. Có thể dùng Creator/Producer để ghi “có dấu hiệu xuất từ Word”; PDF có `/ByteRange` hoặc trường `/Sig` phải được báo là tài liệu đã ký, nhưng không được khẳng định khôi phục file Word gốc hay giữ hiệu lực chữ ký trong DOCX. Word editable dùng tái dựng dòng chảy: giữ khổ/ngắt trang, gom dòng thành đoạn, phục hồi bố cục hai cột và nhận diện bảng `STT` thành bảng Word thật. Bố cục vẫn là gần đúng và phải được mô tả trung thực trên UI.
- Khi sửa PDF → Word, E2E exact-mode phải kiểm tra ảnh neo theo trang, `pgSz`, lề 0, media cho mỗi trang và render LibreOffice xác nhận không có trang trắng/lệch cạnh. Với Word editable dạng công văn, ngoài semantic OOXML phải kiểm tra số trang, hai cột tiêu đề, bảng, nơi nhận và khối ký tên. Không dùng OCR cho PDF đã có lớp chữ chỉ vì metadata Word bị phần mềm ký số ghi đè.
